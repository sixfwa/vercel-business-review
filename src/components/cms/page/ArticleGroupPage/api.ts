...
import {
  ...
  type IContentDataFragment,
  type IContentInfoFragment,
  type Locales,
...
export async function getArticles(
  ...
  const parentGroup =
    (Array.isArray(articlesResult.group?.data) &&
      articlesResult.group?.data[0]) ||
    undefined;
  if (!parentGroup) return { total: 0, items: [] };

  const total = parentGroup.children?.listing?.total ?? 0;
  const items = (parentGroup.children?.listing?.items || [])
    .map((article) => {
      if (!article) return undefined;
      const listItem: ArticleListItem = {
        key:
          ((article as IContentDataFragment)._metadata as IContentInfoFragment)
            .key || "",
        link:
          ((article as IContentDataFragment)._metadata as IContentInfoFragment)
            .url || undefined,
        ...
        published: article._metadata?.published
          ? new Date(article._metadata?.published)
          : new Date(),
      };
      return listItem;
    })
