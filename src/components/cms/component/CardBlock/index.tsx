import {
  ...
  type CardBlockDataFragment,
  type ButtonBlockPropertyDataFragment,
  type ButtonBlockDataFragment,
} from "@/gql/graphql";
 ...
 */
export const CardBlockComponent: CmsComponent<CardBlockDataFragment> = ({
  data: {
    layout: imageLayout = "before",
    image,
    heading = "",
    subheading = "",
  ...
  switch (color) {
    ...
    case "blue":
      additionalClasses.push(
        "bg-azure dark:bg-transparent dark:border-azure dark:border-4"
      );
    ...
    case "orange":
      additionalClasses.push(
        "bg-tangy dark:bg-transparent dark:border-tangy dark:border-4"
      );
      innerClasses.push(
        "text-vulcan prose-h3:text-vulcan prose-h2:text-vulcan"
      );
    ...
    case "green":
      additionalClasses.push(
        "bg-verdansk dark:bg-transparent dark:border-verdansk dark:border-4"
      );
      innerClasses.push(
        "text-vulcan prose-h3:text-vulcan prose-h2:text-vulcan "
      );
    ...
    case "red":
      additionalClasses.push(
        "bg-paleruby dark:bg-transparent dark:border-paleruby dark:border-4"
      );
      innerClasses.push("text-white prose-h3:text-white prose-h2:text-white");
      if (button) {
        buttonClasses.push("btn--light");
    ...
    >
      <div className="w-full h-full grid auto-rows-auto @[80rem]/card:items-center grid-cols-1 gap-16 @[80rem]/card:grid-cols-12">
        ...
        >
          ...
          {(heading || inEditMode) && (
            <CmsEditable as="h2" cmsFieldName="CardHeading">
              {heading}
            </CmsEditable>
          )}
          {(subheading || inEditMode) && (
            <CmsEditable as="h3" cmsFieldName="CardSubHeading">
              {subheading}
            </CmsEditable>
          )}
...
CardBlockComponent.getDataFragment = () => [
  "CardBlockData",
  CardBlockDataFragmentDoc,
];
...
function isButtonBlock(
  toTest?: Record<string, any> | null
): toTest is ButtonBlockPropertyDataFragment {
  return (
    typeof toTest == "object" &&
    toTest != null &&
    (toTest as ButtonBlockPropertyDataFragment).text != null
  );
}
