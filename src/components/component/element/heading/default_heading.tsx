import type {
  HeadingLayoutProps,
  DefaultHeadingLayoutProps,
  HeadingComponent,
} from "./types";
import { type HeadingElementDataFragment } from "@/gql/graphql";
import { extractSettings } from "@remkoj/optimizely-cms-react/components";

export const DefaultHeadingElement: HeadingComponent<
  HeadingElementDataFragment,
  DefaultHeadingLayoutProps
> = ({ data: { headingText }, layoutProps, className, ...containerProps }) => {
  const { headingType, showAs, textAlign, transform } =
    extractSettings(layoutProps);
  const cssClasses: (string | undefined)[] = [className, "flex-initial"];
  const Component =
    showAs == "element"
      ? "p"
      : !headingType || headingType == "plain"
      ? "p"
      : headingType;
...
export function isDefaultTemplate(
  props?: HeadingLayoutProps | null
): props is DefaultHeadingLayoutProps {
