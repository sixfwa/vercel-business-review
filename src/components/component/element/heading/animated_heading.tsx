...
export const AnimatedHeadingElement: HeadingComponent<
...
> = ({ data: { headingText }, layoutProps, className, ...containerProps }) => {
  ...
  return (
    <div
      className={(
        `${className} text-6xl font-extrabold ` + cssClasses.join(" ")
      ).trim()}
      {...containerProps}
    >
...
export function isAnimatedTemplate(
  props?: HeadingLayoutProps | null
): props is AnimatedHeadingLayoutProps {
