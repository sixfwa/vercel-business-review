...
function isButtonBlockFragment(
  toTest: any
): toTest is Schema.IContentDataFragment & Schema.ButtonBlockDataFragment {
...
const SecondaryMenu = forwardRef<HTMLUListElement>((props, ref) => {
  ...
  return (
    <ul ref={ref} className="flex items-center justify-end gap-4">
      {utilityItems.filter(isButtonBlockFragment).map((item) => {
        const link: Schema.LinkDataFragment | undefined =
          item.link ?? undefined;
        const meta: Schema.IContentInfoFragment | undefined =
          item._metadata ?? undefined;
        const key =
          id +
          contentLinkToString(
            meta as InlineContentLink | ContentLink | undefined,
            true
          );
        ...
        const buttonType: ButtonProps["buttonType"] = (item.buttonType ??
          "primary") as ButtonProps["buttonType"];
        const buttonVariant: ButtonProps["buttonVariant"] = (item.variant ??
          "default") as ButtonProps["buttonVariant"];
